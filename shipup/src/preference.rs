...
//!
//! # 模块职责
//! 持久化三类客户端状态：[`UpdatePreference`] 中记录的「已跳过版本」「稍后提醒截止时间」、
//! 稳定的「客户端设备标识」以及「已处理的清单版本序号」，并提供安全的数据目录解析。
//!
//! # 设计原理
//! - **实现初衷**：跳过版本与稍后提醒必须跨进程重启依然生效；而灰度放量分桶与防重放校验
//!   又要求同一台设备拥有稳定且唯一的标识，这些状态天然适合收敛到单一偏好文件中。
//! - **核心优势**：
//!   - 静默期基于绝对 Unix 时间戳计算截止时刻，即使系统停机数天，恢复后依然能正确判定过期；
//!   - 客户端标识首次访问时惰性生成并落盘，避免每次启动都产生新的随机标识导致灰度分桶漂移；
//!   - [`resolve_safe_data_dir`] 在多个候选目录中择优选取可写路径，失败时逐级降级而非直接报错。
//! - **代价与局限**：偏好文件为单文件全量写回，状态规模增长会线性抬升写入成本
//!   （当前字段数量级下可忽略）。
//!
//! # 数据安全
//! 偏好文件不存储任何凭证或密钥；写入采用「先写临时文件再原子重命名」，
//! 避免进程中途退出导致文件被截断为半截 JSON。
