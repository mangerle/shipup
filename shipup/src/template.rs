...
//!
//! # 模块职责
//! 依据 [`TemplateContext`] 中的目标平台、当前版本与发布通道，
//! 把清单端点 URL 中的占位符（`{{target}}`、`{{current_version}}`、`{{channel}}`）展开为真实地址。
//!
//! # 设计原理
//! - **实现初衷**：多平台、多通道发布若为每个组合硬编码一条端点，配置量会随维度乘积爆炸；
//!   模板化后只需维护一份带占位符的端点列表。
//! - **核心优势**：
//!   - 纯字符串替换、无任何 I/O 与状态，可安全地在并发竞速的多个线程中重复调用；
//!   - 未提供的可选上下文（如未配置通道）会被展开为空串而非残留 `{{channel}}` 字面量，
//!     避免把非法 URL 泄漏到网络层。
//! - **代价与局限**：仅支持位置无关的简单占位符替换，不支持条件分支或嵌套表达式。
//!
//! # 使用契约
//! 展开结果必须交由更新器的端点安全门禁（HTTPS / file 协议校验）二次把关，
//! 不得因为「来自模板」而跳过校验。
