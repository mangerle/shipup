...
//!   调用端几乎无法判断「哪些参数可以安全地使用默认值」。
//! - **核心优势**：
//!   - 全部字段都具备安全默认值（如防降级开启、强制验签开启、超时 15 秒、目标平台自动探测），
//!     使得最简用法只需指定「当前版本」与「清单地址」两项；
//!   - 危险选项（明文传输、关闭验签、允许 `file://`）均需显式开启，
//!     默认配置下不可能被静默降级；
//!   - 构建期即完成安全门禁校验，把配置错误提前到启动阶段暴露，而不是等到用户点击「检查更新」才失败。
//! - **代价与局限**：配置一经构建即固化，运行期无法动态调整端点或验签策略；
//!   需要不同策略时必须重新构建实例。
//!
//! # 校验契约
