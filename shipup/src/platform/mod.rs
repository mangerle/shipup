...
//!
//! # 模块职责
//! 把「文件替换」「临时路径规划」「安装器派生」「延迟重启替换」「历史备份清理」五类
//! 强平台相关的能力收敛为统一门面，并按 `cfg(target_os)` 分派到 [`windows`]、[`linux`]、[`macos`] 子模块。
//!
//! # 设计原理
//! - **实现初衷**：跨平台更新最棘手的部分不是网络或签名，而是「如何在程序正在运行时替换它自己」。
//!   三个操作系统的可行路径完全不同，若让上层各写一份判断，必然出现某平台漏改的隐蔽缺陷。
//! - **核心优势**：
//!   - 上层只依赖统一函数签名，平台差异被完全封装在编译期分派的子模块中；
//!   - 临时路径统一遵循「与目标文件同卷」原则，从根本上规避跨文件系统原子重命名失败
//!     （`EXDEV: Cross-device link`）；
//!   - 随机临时目录与文件名令牌均由密码学安全随机源生成，防止恶意进程预占路径实施投毒替换。
//! - **代价与局限**：同卷约束意味着临时目录必须与可执行文件位于同一分区，
//!   当该分区空间紧张时无法自动借用其他分区。
//!
//! # 安全契约
//! [`replace_binary`] 必须保证「替换成功」或「原文件完好」二者之一，
//! 不允许出现目标文件被截断或残缺的中间状态。
