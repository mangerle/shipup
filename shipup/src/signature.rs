//! SHA-256 完整性校验与 Ed25519 数字签名校验模块。
//!
//! # 模块职责
//! 提供内存数据与磁盘文件两类校验入口：
//! - 完整性：[`verify_sha256`]、[`compute_file_sha256_digest`]、[`verify_sha256_file`]；
//! - 真伪性：[`verify_ed25519`] 系列，支持任意一枚公钥通过（`any_key`）与
//!   TUF 风格门限多签（`threshold`）。
//!
//! # 设计原理
//! - **实现初衷**：SHA-256 只能证明「传输未损坏」，无法证明「发布者可信任」；
//!   而 Ed25519 校验需要把整个文件读入内存，对大体积更新包不可接受。两者职责必须分层。
//! - **核心优势**：
//!   - 大文件路径采用流式分块摘要，内存占用恒定在固定缓冲区大小，与文件体积解耦；
//!   - 签名解码与公钥解码的失败被归一为 [`UpdateError::InvalidSignature`]，
//!     避免把非法的 Base64 输入与真正的「签名不匹配」混为一谈；
//!   - `threshold` 变体会强制统计来自**不同**公钥的有效签名数量，杜绝同一密钥重复签名的伪造通过。
//! - **代价与局限**：门限模式要求发布端组织多密钥联合签署，运维成本高于单签。
//!
//! # 安全契约
//! [`MAX_SIGNATURE_PAYLOAD_SIZE`] 约束了单次内存校验允许的最大载荷，
//! 超出后必须改用流式文件校验接口，避免恶意超大输入耗尽内存。
