...
//!
//! # 模块职责
//! 支撑企业内网、隔离网络与移动介质三类离线部署场景：
//! - URI 适配：`file://` 协议判定、路径与 URL 双向转换、相对路径展开；
//! - 仓库自检：[`verify_offline_repository`] 在真正发起更新前逐项核验离线镜像的完整性与一致性。
//!
//! # 设计原理
//! - **实现初衷**：离线介质极易出现「文件缺失」「清单与实际内容不一致」「相对路径基准错位」三类问题，
//!   若等到更新流程中途才发现，用户将面对一条难以理解的底层 I/O 报错。
//! - **核心优势**：
//!   - 路径转换统一处理百分号转义与 Windows 盘符差异，杜绝手工拼装 URL 时的转义遗漏；
//!   - 仓库自检会一次性汇总全部问题项并以报告形式返回，便于运维批量修复而非逐个试错；
//!   - 与 `blocking` / `async` 特性完全解耦，关闭网络特性时依然可用于纯离线校验。
//! - **代价与局限**：`file://` 协议默认关闭，必须由调用方显式开启，
//!   以免恶意相对路径注入被静默放行。
//!
//! # 安全契约
//! 相对 `file://` 路径展开必须以清单所在目录为基准，严禁以进程当前工作目录为基准，
//! 否则切换启动目录即可让更新器读取任意本地文件。
