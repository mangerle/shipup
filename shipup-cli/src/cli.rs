...
//!
//! # 模块职责
//! 以 `clap` 派生宏声明顶层命令 [`Cli`] 与子命令枚举 [`Commands`]，
//! 并为每个子命令定义独立的强类型参数结构体（如 [`ReleaseArgs`]、[`VerifyArgs`]）。
//!
//! # 设计原理
//! - **实现初衷**：把「参数解析」与「业务执行」彻底分离。
//!   本模块只负责把命令行文本转换为强类型参数，任何分支逻辑都不在此处出现。
//! - **核心优势**：
//!   - 每个子命令拥有专属参数结构体，避免使用一个「大而全」的参数结构承载所有子命令的并集字段
//!     （那种做法会让「哪个参数属于哪个命令」变得不可靠）；
//!   - 借助派生宏自动生成 `--help`、参数校验与补全脚本，且解析失败时由 `clap` 统一输出中文提示。
//! - **代价与局限**：参数结构体与子命令枚举需要成对维护，新增子命令时两处都要同步更新。
//!
//! # 契约
//! 本模块只描述「接受什么参数」，不保证参数语义上的合法性；
//! 例如路径是否存在、密钥是否可解析等检查，一律由对应的 `handle_*` 处理函数负责。
...
/// 命令行顶层参数容器。
///
/// 当前仅承载子命令枚举，后续若引入全局开关（如 `--verbose`、`--offline`），
/// 应在此结构体上追加字段，避免污染各子命令的参数结构体。
#[command(
    name = "shipup-cli",
    author,
    version,
