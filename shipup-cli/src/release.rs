...
pub(crate) fn load_or_init_manifest(
    manifest_path: &Path,
    args: &ReleaseArgs,
    entry: &ManifestReleaseEntry,
) -> Result<Manifest> {
...
pub(crate) fn save_manifest_file(manifest_path: &Path, manifest: &Manifest) -> Result<()> {
    let json_output =
...
mod tests {
    use super::*;
    use crate::cli::{InspectArgs, VerifyArgs};
...
"#;
    ...
    #[test]
    fn test_compute_payload_integrity_matches_sha256() -> Result<()> {
