//! 校验相关命令模块：Manifest 核验、清单展示与离线仓库审计。
//!
//! # 模块职责
//! 实现 `verify`、`inspect`、`verify-repo` 三个子命令：
//! 核验清单签名与包体一致性、以可读形式展示清单内容、审计离线镜像仓库的完整性。
//!
//! # 设计原理
//! - **实现初衷**：发布环节的绝大多数事故并非出在签名算法本身，
//!   而是「清单里写的哈希与实际上传的包体对不上」；这类问题必须在发布流水线内被拦截，
//!   而不是等客户端更新失败后才暴露。
//! - **核心优势**：
//!   - 核验流程复用客户端完全相同的校验接口，做到「发布端能过、客户端才可能过」；
//!   - 离线仓库审计一次性遍历全部平台包体并汇总问题清单，支持在断网环境中完成合规检查；
//!   - 展示命令只读不写，可安全地用于排查线上清单。
//! - **代价与局限**：增量校验未实现，包体数量较多时需要完整遍历，耗时与总体积成正比。
//!
//! # 安全契约
//! 校验一律以「清单中的哈希」为基准，不得反过来以本地实际文件为准去更新清单，
//! 否则会掩盖真实的发布事故。
